        ...
        for i in range(msg_limit):
            ...
            if msg_count != 0 and i == msg_limit - 1:
                try:
                    ...
                    raise urllib2.HTTPError(url=new_url, code=420, hdrs=None, msg='Enhance Your Calm', fp=None)
...
class AppWindow(QtGui.QDialog):
    ...
    def get_chat(self, token, chat_type, chat_ID, msg_ID=None):
        ...
        try:
            i_json = get_json(url)
            raise urllib2.HTTPError(url=url, code=420, hdrs=None, msg='Enhance Your Calm', fp=None)
        except urllib2.HTTPError, err:
